...
class TokenizerTest(unittest.TestCase):
    ...
    def test_tokenize_not_alphabetical_symbols_at_the_end_input(self):
        """check the returned value if an input contains non-alphabetic symbols at the end."""
        actual_result = Tokenizer().tokenize('Token._?')
        instance_of_actual_result = actual_result[0]
        self.assertEqual(instance_of_actual_result.position, 0)
        self.assertEqual(instance_of_actual_result.string_representation, 'Token')

    def test_tokenize_alphabetical_symbol_at_the_end_input(self):
        """check the returned value if the end of input is alphabetical symbol."""
        actual_result = Tokenizer().tokenize('Token')
        instance_of_actual_result = actual_result[0]
        self.assertEqual(instance_of_actual_result.position, 0)
        self.assertEqual(instance_of_actual_result.string_representation, 'Token')

    def test_tokenizer_not_alphabetical_symbols_at_the_beginning_of_input(self):
        """check the returned value if an input contains non-alphabetic symbols at the beginning."""
        actual_result = Tokenizer().tokenize('--token')
        instance_of_actual_result = actual_result[0]
        self.assertEqual(instance_of_actual_result.position, 2)
        self.assertEqual(instance_of_actual_result.string_representation, 'token')

    def test_tokenizer_other_symbols_at_the_beginning_and_the_end_of_input(self):
        """check the returned value if an input contains non-alphabetic symbols at the beginning and at the end."""
    ...
    def test_tokenizer_space_after_punctuation(self):
        """check the returned value if the input has space after punctuation."""
        ...
        for i in actual_result:
            ...
            p += 1
            self.assertEqual(i.string_representation, values_for_comparison[p])
            p += 1
